...
function makeMove(args) {
  ...
  * method to think.
  */
 var [active, holdPiece, nextPiece, jsobj, _x, _y, holdX, holdY, _rot] = args;
 let resolve = {};
 let json = (piece) => {
  let le = jsobj || {};
  le.active = piece;
 ...
 if (holdPiece !== void 0) {
  otherPiece = evaluatePieces(json(holdPiece));
 } else otherPiece = evaluatePieces(json(nextPiece));

 let best = activePiece.a;
 let bestPieceIndex = active;
 let life = 20;
 let iter = activePiece.iterations;
 if (activePiece.a.score < otherPiece.a.score) {
  best = otherPiece.a;
  iter = otherPiece.iterations;
  bestPieceIndex = holdPiece !== void 0 ? holdPiece : nextPiece;
      ...
      for (let TX = 0; TX < len1; TX++) {
       for (let TY = 0; TY < len2; TY++) {
        for (var TE = 0; TE < len3 && PIECE !== 6; TE++) {
         if (PIECE !== 6 && mt[TX][TY] > 0 && mt[TX][TY] !== 8 && TX + mx == jsobj.tPrevent[TE][0] && TY + my == jsobj.tPrevent[TE][1]) {
          failedTspin++;
         }
        }
